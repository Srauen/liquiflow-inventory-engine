...
import { BrainCircuit, Send, Sparkles, Cpu, ShieldCheck } from 'lucide-react';
      ...
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-gray-400 mt-2">
            Local enterprise-grade inventory reasoning. No cloud dependency.
            <span className="block text-xs text-neon-emerald mt-1 font-mono uppercase flex items-center gap-2">
              <ShieldCheck className="w-3 h-3" /> Status: On-Device Processing Active
            </span>
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 flex-1">
        ...
        <div className="lg:col-span-1 space-y-6">
          <GlassCard className="h-full flex flex-col border-white/10 bg-black/40">
            <h3 className="font-black text-xs uppercase tracking-[0.2em] mb-4 text-neon-blue">Logic Parameters</h3>
            <div className="flex-1">
              <textarea 
                ...
                placeholder="Example: 'Analyze the risk of holding SKU-2077 vs wholesale liquidation.'"
                value={query}
            ...
            <div className="mt-4 flex justify-between items-center">
              ...
              >
                {isThinking ? 'PROCESSING...' : 'RUN LOGIC'} <Send className="w-4 h-4" />
              </button>
            </div>
          </GlassCard>
        </div>
          ...
          <GlassCard className="h-full flex flex-col relative overflow-hidden bg-gradient-to-br from-white/5 to-black/80 border-white/10">
            {isThinking && (
              <div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-black/80 backdrop-blur-xl">
                ...
                <p className="text-neon-blue font-mono animate-pulse uppercase tracking-[0.2em] text-[10px] font-black">Running Local Neural Cycles...</p>
                <p className="text-[9px] text-gray-600 mt-2 font-mono uppercase">Parsing Multi-Variable Liquidity Matrices</p>
              </div>
            )}
            
            <div className="flex items-center gap-2 mb-6 border-b border-white/5 pb-4">
              <BrainCircuit className="w-5 h-5 text-neon-violet" />
              <h3 className="font-black text-xs uppercase tracking-[0.2em] text-white">Strategic Logic Output</h3>
            </div>
              ...
              {response ? (
                <div className="prose prose-invert max-w-none">
                  <p className="font-mono text-[9px] text-gray-600 mb-4 uppercase tracking-widest">
                    SESSION_ID: {Math.random().toString(16).slice(2, 10).toUpperCase()} // CORE: NEURAL-EDGE-LOCAL
                  </p>
                ...
                <div className="h-full flex flex-col items-center justify-center text-gray-800">
                  ...
                  <p className="font-mono text-[10px] uppercase tracking-[0.3em] font-black">Awaiting logic stream...</p>
                </div>
              )}
            </div>
          </GlassCard>
